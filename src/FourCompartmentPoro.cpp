...
void FourCompartmentPoro::setMaterialConstants()
{
    // geometry constants
    rV      = 30.0e-3;    // m
    rS      = 100.0e-3;   // m
...
void FourCompartmentPoro::setGridConstants( int m )
{
    int i;
    // grid properties
    ...
    for ( i=0; i<J; i++ ) { r[i] = rV + i*dr; }
}

void FourCompartmentPoro::setTimeConstants( double INIT_TIME_SECS, double FINAL_TIME_SECS, double DT_SECS )
{
    dt      = DT_SECS;                  // time step size
    tf      = FINAL_TIME_SECS;          // final solution time
    t0      = INIT_TIME_SECS;           // initial time
    t       = t0;                       // current time
    N       = int((tf-t0+dt/10.)/dt)+1; // number of time steps
}

void FourCompartmentPoro::initializeSystem()
{
    int i;
    int j;
    int n = 5;
    ...
    for ( i=0; i<n*J; i++ ) { for ( j=0; j<n*J; j++ ) { A(i, j) = 0.0; } }
}

void FourCompartmentPoro::createTransientDataFile ()
{
...
void FourCompartmentPoro::buildSystem ()
{
    // == build the A matrix ==== //
    int i;
    ...
    A(3*J, 3*J)          = M_PI*pow(d,4)/128./mu_e/L + 4.*M_PI*kappa_e*pow(r[0]+x[0],2)/dr;
...
void FourCompartmentPoro::saveWallData ()
{
...
void FourCompartmentPoro::saveTransientData ()
{
    std::ofstream fs;
...
void FourCompartmentPoro::Initialize ( int m, double INIT_TIME_SECS, double FINAL_TIME_SECS, double DT_SECS, bool saveT, std::string bName )
{
    saveTrans = saveT;
    baseName = bName;
    // remove trailing spaces from the base name
    size_t endpos = baseName.find_last_not_of(" ");
    if( std::string::npos != endpos ) baseName = baseName.substr( 0, endpos+1 );

    setMaterialConstants();
    setGridConstants(m);
    setTimeConstants(INIT_TIME_SECS, FINAL_TIME_SECS, DT_SECS);
    initializeSystem();
    if (saveTrans) { createTransientDataFile(); }
}

void FourCompartmentPoro::SetArteriolConstants( double Aa, double Ba, double kA, double muA )
{ 
    A_a     = Aa;
    B_a     = Ba;
...
void FourCompartmentPoro::SetCapillaryConstants( double Ac, double Bc, double kC, double muC, double kCE )
{ 
    A_c     = Ac;
    B_c     = Bc;
...
void FourCompartmentPoro::SetVenousConstants( double Av, double Bv, double kV, double muV )
{ 
    A_v     = Av;
    B_v     = Bv;
...
void FourCompartmentPoro::SetTransferConstants( double gammaAC, double gammaCE, double gammaCV, double gammaEV )
{
    gamma_ac = gammaAC;
    gamma_ce = gammaCE;
    gamma_cv = gammaCV;
...
void FourCompartmentPoro::Solve ()
{
    int i, noPrint;
    noPrint = 1;
    for ( i=0; i<N; i++ )
